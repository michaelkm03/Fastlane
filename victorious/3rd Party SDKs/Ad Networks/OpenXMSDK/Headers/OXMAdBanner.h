...
/** Ad banner component, which manages the process of loading and displaying of banner advertisements.
 Provides the simplest integration scenario. Can be easily integrated with interface builder.
 */
...
@property (nonatomic, weak) NSObject<OXMAdBannerDelegate>* adBannerDelegate;
...
/** Starts loading ads from OpenX server sending it auto-detected and user-provided parameters in request.
 ...
 The banner will automatically load new ads based on the current ad change interval. Set your desired parameters on the request before calling this.
 */
- (void) startLoading;

/** Stop loading ads based on the ad change interval. */
- (void) stopLoading;
...
/**
 This protocol contains delegate methods for controlling ad loading process, and handling ad banner actions.
 */
...
/** This method is invoked when some action was initiated by the user.
 ...
 -adBannerActionDidBegin: will be called after the transition is complete.
