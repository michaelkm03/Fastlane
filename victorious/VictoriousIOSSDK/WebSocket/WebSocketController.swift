...
public class WebSocketController: WebSocketDelegate, ForumNetworkSourceWebSocket, WebSocketEventDecoder, WebSocketPongDelegate {
    ...
    public func websocketDidDisconnect(socket: WebSocket, error: NSError?) {
        ...
        let disconnectEvent = webSocketEventFromDisconnect(error)

        dispatch_async(dispatch_get_main_queue()) { [weak self] in
            if let disconnectEvent = disconnectEvent {
    ...
    private func webSocketEventFromDisconnect(error: NSError?) -> WebSocketEvent? {
        var disconnectEvent: WebSocketEvent?

        if let error = error, let errorMessageData = error.localizedDescription.dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false) {
            let errorJSON = JSON(data: errorMessageData)
            if let webSocketError = WebSocketError(json: errorJSON, didDisconnect: true) {
                disconnectEvent = .Disconnected(webSocketError: webSocketError)
            }
        }
