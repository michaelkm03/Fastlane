...
protocol ChatInterfaceDataSource: UICollectionViewDataSource {
    ...
    /// The chat feed items that are unstashed and active in the feed.
    var unstashedItems: [ChatFeedContent] { get }
...
extension ChatInterfaceDataSource {
    var itemCount: Int {
        return unstashedItems.count + pendingItems.count
    }
    
    func content(at index: Int) -> ChatFeedContent {
        if index < unstashedItems.count {
            return unstashedItems[index]
        }
        else {
            return pendingItems[index - unstashedItems.count]
        }
    }
