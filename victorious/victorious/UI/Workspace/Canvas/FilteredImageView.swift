...
class FilteredImageView: GLKView {
    ...
    var inputImage: UIImage? {
        didSet {
//            let size = CGSize(width: bounds.width / UIScreen.mainScreen().scale, height: bounds.height / UIScreen.mainScreen().scale)
            if let inputImage = inputImage {
                self.scaledImage = Toucan.Resize.resizeImage(inputImage, size: CGSize(width: 260.0, height: 260.0) , fitMode: .Clip)
            }
        }
    }
