...
- (void)followUser:(VUser *__nonnull)user withAuthorizedBlock:(void (^ __nullable)(void))authorizedBlock andCompletion:(VFollowEventCompletion __nonnull)completion fromViewController:(UIViewController *__nullable)viewControllerToPresentOn withScreenName:(NSString *__nullable)screenName
{
    ...
    [followResponder followUser:user withAuthorizedBlock:authorizedBlock andCompletion:completion fromViewController:viewControllerToPresentOn withScreenName:[NSString stringWithFormat:@"%@.%@", VFollowSourceScreenStream, screenName]];
}

- (void)unfollowUser:(VUser *__nonnull)user withAuthorizedBlock:(void (^ __nullable)(void))authorizedBlock andCompletion:(VFollowEventCompletion __nonnull)completion
{
    id <VFollowResponder> followResponder = [self.nextResponder targetForAction:@selector(unfollowUser:withAuthorizedBlock:andCompletion:) withSender:nil];
    NSAssert(followResponder != nil, @"VStreamCollectionViewController needs a follow repsonder up it's responder chain to send messages to.");
    [followResponder unfollowUser:user withAuthorizedBlock:authorizedBlock andCompletion:completion];
}
