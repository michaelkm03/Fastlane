#!/bin/sh
#
# update-xib-strings.sh - automatically extract translatable strings from storyboards and xibs and update strings files
...
do
    ...
    if find $storyboardPath -prune -newer $baseStringsPath -print | grep -q .; then
        ...
        newBaseStringsPath=$(echo "$storyboardPath" | sed -E "s/($storyboardExt|$xibExt)$/$newStringsExt/")
