            ...
            andParent:(VComment*)parent
{
    ...
    if (!sequence.remoteId) //Need this or we should quit
    {
...
+ (void)removeComment:(VComment*)comment withReason:(NSString*)removalReason
{
    ...
    if (!comment.remoteId) //Need this or we should quit
    {
    ...
    [requestOperation setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation,
                                                      RKMappingResult *mappingResult)
     {
         ...
         {
             RKLogInfo(@"Removing comment %@ from core data because of /api/comment/remove", commentToRemove.remoteId);
